...
There is no automated test suite. Before you finish a change, run `npm run build` and `npm run lint` and make sure both pass with no new warnings. Test behaviour by hand in an Obsidian vault (see [Manual testing](#manual-testing)).
...
- **Command IDs.** IDs are derived from names: `apply-rule-<slug>` and `run-sequence-<slug>`. The fixed commands are `quick-find-replace` and `revert-last-quick-action`. Users bind hotkeys to these IDs, so do not change how they are derived.
...
- **Style.** Use 4-space indentation in TypeScript and tabs in `manifest.json` and `versions.json`. Write JSDoc comments that explain why the code is the way it is, not what it does, and match the comment density of the surrounding code. Promise-returning calls from sync callbacks use `void`.

## Changelog, docs, and releases

- Record user-facing changes in `CHANGELOG.md` under the top version heading, using the existing sections (`### New features`, `### UI/UX enhancements and bug fixes`, `### Other`).
- When you add or change a user-facing feature, update both `README.md` and `README_RU.md`.
