...
3. Применение команд только к выделенному тексту. Включите в настройках плагина опцию «Применять быстрые команды к выделенному тексту»: после этого любая команда, вызванная из палитры команд, сочетанием клавиш или из контекстного меню заметки, будет применена только к текущему выделению. Если ничего не выделено, команда применяется ко всей заметке. Команды из контекстного меню файла или папки всегда применяются к файлам целиком.

4. Экспорт и импорт быстрых команд с помощью опции «Резервное копирование и восстановление команд» в конце раздела «Основные» настроек плагина. Экспорт сохраняет все ваши команды в файл JSON и доступен только на компьютере, поскольку мобильное приложение не может сохранить файл таким способом. Импорт добавляет команды из такого файла и никогда не перезаписывает уже имеющиеся: импортируемая команда с уже занятым именем добавляется под новым именем, а уже существующая команда пропускается.

5. Отмена последней быстрой команды через команду «Отменить последнюю быструю команду» в палитре команд. Она отменяет последнее применение целиком, включая применение к нескольким файлам или ко всей папке, и позволяет сделать только один шаг назад, а не хранит историю. Файл не будет затронут, если он был изменён после применения команды, поэтому ваши последующие правки никогда не будут перезаписаны. Данные для отмены хранятся только в памяти: они теряются при перезапуске Obsidian или перезагрузке плагина.
